...
func Open(path string) (*Store, error) {
	...
	s := &Store{DB: db}; if err := s.Migrate(); err != nil { db.Close(); return nil, err }; return s, nil
}
...
func (s *Store) SaveForums(nodes []flashback.ForumNode) error {
	...
	for _, n := range nodes { _, err = tx.Exec(`INSERT INTO forums(id,title,url,parent_id,depth,sort_order,has_children,browsable,last_seen_at) VALUES(?,?,?,?,?,?,?,? ,CURRENT_TIMESTAMP) ON CONFLICT(url) DO UPDATE SET title=excluded.title,parent_id=excluded.parent_id,depth=excluded.depth,sort_order=excluded.sort_order,has_children=excluded.has_children,last_seen_at=CURRENT_TIMESTAMP`, n.ID,n.Title,n.URL,n.ParentID,n.Depth,n.SortOrder,n.HasChildren,n.Browsable); if err != nil { return err } }
	return tx.Commit()
}

func (s *Store) SaveThreads(forumID string, rows []flashback.ThreadSummary) error {
	tx, err := s.DB.Begin(); if err != nil { return err }; defer tx.Rollback()
	for i, r := range rows { _, err = tx.Exec(`INSERT INTO threads(id,title,url,forum_id,replies,views,last_post_at,last_post_author,sticky,page_count,last_seen_at) VALUES(?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP) ON CONFLICT(id) DO UPDATE SET title=excluded.title,url=excluded.url,forum_id=excluded.forum_id,replies=excluded.replies,views=excluded.views,last_post_at=excluded.last_post_at,last_post_author=excluded.last_post_author,sticky=excluded.sticky,page_count=excluded.page_count,last_seen_at=CURRENT_TIMESTAMP`,r.ID,r.Title,r.URL,forumID,r.Replies,r.Views,r.LastPostAt,r.LastPostAuthor,r.Sticky,r.PageCount); if err != nil { return err }; _, err = tx.Exec(`INSERT INTO forum_threads(forum_id,thread_id,position) VALUES(?,?,?) ON CONFLICT(forum_id,thread_id) DO UPDATE SET position=excluded.position`,forumID,r.ID,i); if err != nil { return err } }
	return tx.Commit()
}

func (s *Store) SavePage(page flashback.ParsedPage) error {
	tx, err := s.DB.Begin(); if err != nil { return err }; defer tx.Rollback()
	_, err = tx.Exec(`INSERT INTO threads(id,title,url,page_count,last_seen_at) VALUES(?,?,?,?,CURRENT_TIMESTAMP) ON CONFLICT(id) DO UPDATE SET title=CASE WHEN excluded.title!='' THEN excluded.title ELSE threads.title END,page_count=MAX(threads.page_count,excluded.page_count),last_seen_at=CURRENT_TIMESTAMP`,page.ThreadID,page.Title,page.SourceURL,page.MaxPage); if err != nil { return err }
	for _, p := range page.Posts { _, err = tx.Exec(`INSERT INTO posts(id,thread_id,author,timestamp,page,position,text,raw_text,source_url) VALUES(?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET author=excluded.author,timestamp=excluded.timestamp,page=excluded.page,position=excluded.position,text=excluded.text,raw_text=excluded.raw_text,source_url=excluded.source_url,last_seen_at=CURRENT_TIMESTAMP`,p.ID,p.ThreadID,p.Author,p.Timestamp,p.Page,p.Position,p.Text,p.RawText,p.SourceURL); if err != nil { return err }; _, err = tx.Exec(`DELETE FROM post_search WHERE post_id=?`,p.ID); if err != nil { return err }; _, err = tx.Exec(`INSERT INTO post_search(post_id,thread_id,author,text) VALUES(?,?,?,?)`,p.ID,p.ThreadID,p.Author,p.Text); if err != nil { return err } }
	return tx.Commit()
}

func (s *Store) TrackedThreads() (*sql.Rows, error) { return s.DB.Query(`SELECT id,title,replies,last_post_at FROM threads ORDER BY last_seen_at DESC`) }
func (s *Store) Forums(parent string) (*sql.Rows, error) { if parent == "" { return s.DB.Query(`SELECT id,title,url,has_children FROM forums WHERE parent_id IS NULL ORDER BY sort_order`) }; return s.DB.Query(`SELECT id,title,url,has_children FROM forums WHERE parent_id=? ORDER BY sort_order`, parent) }
func (s *Store) Posts(threadID string) (*sql.Rows, error) { return s.DB.Query(`SELECT id,author,timestamp,text FROM posts WHERE thread_id=? ORDER BY page,position,id`, threadID) }
