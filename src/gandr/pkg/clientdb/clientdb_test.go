...
func TestChatMessagesPersistAndDeduplicate(t *testing.T) {
	path := filepath.Join(t.TempDir(), "client.db")
	...
	var hash, channel, sender [32]byte
	hash[0], channel[0], sender[0] = 1, 2, 3
	want := ChatMessage{Hash: hash, ChannelID: channel, Sender: sender, Content: "sparat meddelande", At: 123, Local: true}
	if err := db.SaveChatMessage(want); err != nil {
		t.Fatal(err)
	}
	if err := db.SaveChatMessage(want); err != nil {
		t.Fatal(err)
